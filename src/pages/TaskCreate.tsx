...
function TaskCreate({ mode = 'create' }: TaskCreateProps) {
  ...
  }, [firstGroup, formData.taskType, user?.uid]);
  ...
  useEffect(() => {
    if (mode === 'edit' && existingTask) {
      ...
      setFormData({
        ...
        selectedGroupId: existingTask.groupId || firstGroup?.id || '',
  ...
  const validateForm = () => {
    ...
    // 담당자 선택 검증 로직 개선
    if (formData.taskType === 'personal') {
      // 개인 할일일 때는 현재 사용자가 담당자
      if (!user?.uid) {
        newErrors.assigneeId = '사용자 정보를 찾을 수 없습니다.';
      }
    } else {
      // 그룹 할일일 때는 담당자 선택 필수
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
                ...
                {/* 그룹 할일 옵션 */}
                <WaveButton
                  variant={formData.taskType === 'group' ? 'primary' : 'ghost'}
                  onClick={() => {
                    setFormData(prev => ({
                      ...prev,
                      taskType: 'group',
              ...
              {/* 그룹 선택 (그룹 할일일 때만 표시) */}
              {formData.taskType === 'group' && groups && groups.length > 0 && (
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <div>
                <Typography.Label className="text-white/90 mb-4 flex items-center gap-2">
                  <User size={16} />
                  {formData.taskType === 'personal' ? '담당자' : '담당자 선택'}
                  <span className="text-red-400">*</span>
                </Typography.Label>
              ...
              <div className="space-y-4 p-5 bg-background/10 backdrop-blur-sm rounded-xl animate-fadeIn">
                ...
                <div className="p-4 bg-gradient-to-r from-indigo-500/20 to-purple-500/20 backdrop-blur-sm rounded-xl border border-white/20">
                  <Typography.Body className="text-white flex items-center gap-2">
                    ...
                    {formData.recurring.frequency === 'weekly' && '주'}
                    {formData.recurring.frequency === 'monthly' && '개월'}
                    {formData.recurring.frequency === 'yearly' && '년'}마다 반복
                    ...
                    <WaveButton
                      ...
                      onClick={() => {
                        if (formData.reminders.includes(value)) {
                          setFormData(prev => ({
                            ...prev,
                            reminders: prev.reminders.filter(r => r !== value),
                          }));
                        } else {
                ...
                <Typography.Caption className="text-white flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm leading-relaxed space-y-2">
                      ...
                      <div className="flex items-start gap-2">
                        <span className="text-yellow-400 font-bold">•</span>
                        <span>동일한 시간의 알림은 중복 설정되지 않습니다</span>
                      </div>
                    </div>
                  </div>
                </Typography.Caption>
                ...
                <div className="relative">
                  ...
                  {formData.estimatedMinutes && (
                    <Typography.Caption className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground">
                      {Math.floor(parseInt(formData.estimatedMinutes) / 60)}시간{' '}
                      {parseInt(formData.estimatedMinutes) % 60}분
                    </Typography.Caption>
                  )}
                </div>
              </div>
