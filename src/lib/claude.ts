...
const getClaudeConfig = () => {
  const config = {
    apiKey:
      import.meta.env.VITE_CLAUDE_API_KEY || import.meta.env.CLAUDE_API_KEY,
  ...
  if (!config.apiKey && config.enabled) {
    logger.warn(
      'Claude AI is enabled but API key is missing. Please set VITE_CLAUDE_API_KEY in your environment variables.'
    );
  }
...
export const anthropic = config.apiKey
  ? new Anthropic({
...
export const claudeAIService: ClaudeAIService = {
  async generateTaskSuggestions(input: string): Promise<TaskSuggestion[]> {
    if (!anthropic || !config.enabled) {
      logger.warn('claude', 'Claude AI is not available');
      return [];
    }
    ...
    try {
      ...
      logger.error('claude', 'task suggestions error', error);
      return [];
    }
  },
  ...
  async categorizeTask(title: string, description?: string): Promise<string> {
    ...
    try {
      ...
      logger.error('claude', 'categorization error', error);
      return 'other';
    }
  },
  ...
  async improveTaskDescription(
    ...
    try {
      ...
      logger.error('claude', 'description improvement error', error);
      return description || '';
    }
  },
  ...
  async generateSubtasks(
    ...
    try {
      ...
      logger.error('claude', 'subtasks generation error', error);
      return [];
    }
  },
  ...
  async estimateTaskDuration(
    ...
    try {
      ...
      logger.error('claude', 'duration estimation error', error);
      return 30;
    }
  },
  ...
  async suggestTaskPriority(
    ...
    try {
      ...
      logger.error('claude', 'priority suggestion error', error);
      return 'medium';
    }
  },
};
