...
export function ProfileSection({
  ...
  const handleFieldSave = async (fieldKey: string) => {
    ...
    try {
      ...
      // 로컬 상태 업데이트
      onUpdate({ type: 'UPDATE_PROFILE', payload: { [fieldKey]: localEditData[fieldKey] } });
      ...
      <div className="field-group">
        ...
        {isFieldEditing ? (
          ...
          <div className="field-display">
            <p className={`py-2 font-pretendard font-medium ${
  ...
  return (
    <div className="space-y-6">
      ...
      <GlassCard variant="medium" className="p-6">
        <SettingsGroup title="프로필 정보" description="개인 정보를 관리하세요">
          {/* 헤더 섹션 */}
              ...
              <GlassCard variant="light" className="p-4">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {renderField('displayName', '이름', User, '이름을 입력하세요')}
                    {renderField('email', '이메일', Mail, '', 'email')}
                    {renderField('phone', '전화번호', Phone, '010-0000-0000', 'tel')}
                    {renderField('location', '위치', MapPin, '서울, 대한민국')}
                  </div>

                  {/* 자기소개 필드 - 전체 너비 */}
                  {renderField('bio', '자기소개', Globe, '간단한 자기소개를 작성해주세요', 'textarea')}
